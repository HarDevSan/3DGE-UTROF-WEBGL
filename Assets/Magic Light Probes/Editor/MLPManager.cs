...
namespace MagicLightProbes
{
    public class MLPManager : EditorWindow
    {
        ...
        public static void AddVolume(List<MagicLightProbes> allGroups, Vector3 position = new Vector3(), float avaragedSize = 0)
        {
            ...
            GameObject newProbesVolume;

            if (currentPrefabStage != null)
            {
                MagicLightProbes.CreateHierarchyStructure(null);
                mlpGroupsObject = MagicLightProbes.CreateHierarchyStructure(currentPrefabStage);
                
                newProbesVolume = new GameObject("MLP Group " + allGroups.Count, typeof(MagicLightProbes));
                newProbesVolume.GetComponent<MagicLightProbes>().isInPrefab = true;
            }
            else
            {
                mlpGroupsObject = MagicLightProbes.CreateHierarchyStructure(null);
