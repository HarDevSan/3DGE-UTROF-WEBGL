...
namespace MagicLightProbes
{
    ...
    {       
        ...
        private static Vector2 excludedLightsScrollViewPosition;
        private static Vector2 groundObjectsScrollViewPosition;
                ...
                switch (magicLightProbes.selectedTab)
                {
                    case 0:
                        ...
                        if (!magicLightProbes.useVolumeBottom)
                        {
                            ...
                            if (!globalGroundObjectsUsed)
                            {
                                ...
                                groundObjectsScrollViewPosition = GUILayout.BeginScrollView(groundObjectsScrollViewPosition, 
                                    magicLightProbes.groundAndFloorObjects.Count > 5 ? GUILayout.MaxHeight(200) : GUILayout.MaxHeight(0));
                    ...
                    case 1:
                        ...
                        if (magicLightProbes.workflow == MagicLightProbes.Workflow.Advanced)
                        {
                            ...
                            {
                                GUILayout.Label(
                                    MLPTooltipManager.MainComponent.GetParameter("Exclude Mode",
                                        MLPTooltipManager.MainComponent.Tabs.CullingOptions), GUILayout.MinWidth(200));
                                magicLightProbes.excludingLightsMode =
                                    (MagicLightProbes.ExcludingLightsMode) EditorGUILayout.EnumPopup(
                                        magicLightProbes.excludingLightsMode, GUILayout.MaxWidth(250));
                            }

                            if (magicLightProbes.excludedLights.Count > 5)
                            {
                                excludedLightsScrollViewPosition = GUILayout.BeginScrollView(excludedLightsScrollViewPosition, GUILayout.MaxHeight(200));
                            }
                            ...
                            for (int i = 0; i < magicLightProbes.excludedLights.Count; i++)
                            {
                                ...
                                {
                                    magicLightProbes.excludedLights.RemoveAt(i);
                                    continue;
                                }
                                ...
                                GUILayout.Label(magicLightProbes.excludedLights[i].name, GUILayout.MinWidth(200));

                                if (GUILayout.Button("Select Light"))
                                {
                                    Selection.activeGameObject = GameObject.Find(magicLightProbes.excludedLights[i].name);
                                }
                            ...
                            using (new GUILayout.HorizontalScope())
                            {
                                if (magicLightProbes.excludedLights.Count > 0)
                                {
                                    GUILayout.FlexibleSpace();

                                    if (GUILayout.Button("Clear List", GUILayout.MaxWidth(100)))
                                    {
                                        magicLightProbes.excludedLights.Clear();
                                    }
                                }
                            }
                            ...
                            using (new GUILayout.VerticalScope())
                            {
                                ...
                                if (dragDropArea.Contains(Event.current.mousePosition))
                                {
                                    ...
                                    else if (Event.current.type == EventType.DragPerform)
                                    {
                                        for (int i = 0; i < DragAndDrop.objectReferences.Length; i++)
                                        {
                                            GameObject newExcludedLight = DragAndDrop.objectReferences[i] as GameObject;

                                            if (newExcludedLight != null)
                                            {
                                                MLPLight mlpLight = newExcludedLight.GetComponent<MLPLight>();
                                                ...
                                                if (!magicLightProbes.excludedLights.Contains(mlpLight))
                                                {
                                                    magicLightProbes.excludedLights.Add(mlpLight);
        ...
        private static void CreateSettingsAsset(MagicLightProbes magicLightProbes)
        {
            ...
            projectRelativePath += "Magic Light Probes/Presets/" + SceneManager.GetActiveScene().name + ".asset";
            ...
            List<MagicLightProbes> allGroups)
        {
            ...
            switch (selectedTab)
            {
                ...
                case 1:
                    ...
                    using (new GUILayout.HorizontalScope())
                    {
                        GUILayout.Label("Total Volumes: ", GUILayout.MinWidth(100));
                    ...
                    using (new GUILayout.HorizontalScope())
                    {
                        ...
                        GUILayout.Label("Total Probes: ", GUILayout.MinWidth(100));
                    ...
                    using (new GUILayout.HorizontalScope())
                    {
                        GUILayout.Label(MLPTooltipManager.MainComponent.GetParameter("Skip Calculated", MLPTooltipManager.MainComponent.Tabs.BasicParameters), GUILayout.MinWidth(100));
                        manager.skipCalculated = EditorGUILayout.Toggle(manager.skipCalculated, GUILayout.MaxWidth(250));
                        GUILayout.FlexibleSpace();
                    }
